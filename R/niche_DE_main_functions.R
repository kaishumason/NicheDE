...
niche_DE = function(object,num_cores,outfile, C = 150,M = 10,gamma = 0.8,print = T,Int = T,batch = T,self_EN = F,G = 1){
  ...
  for(sig in object@sigma){
    ...
    for(I in c(1:length(index_chunks))){
      ...
      results_chunk = foreach::foreach(i = 1:NC,.export = c("constant_param","ngene","niche_DE_core","counts_chunk"))%dopar%{
        vec = counts_chunk[,i]
    ...
    rm(list=ls()[! ls() %in% c("object","counter","nb_lik","niche_DE_core","C","M","gamma","valid","cores","Int","batch","print","cluster","nicheDE","self_EN","G","num_cores","outfile")])
    gc()
  }
